...
(use-package
 ...
 (let ((work-notes (expand-file-name "notes.el" user-emacs-directory)))
   (if (file-exists-p work-notes)
       ;; Load work-related notes and announce it.
 ...
 (let ((templates (expand-file-name "templates.el" user-emacs-directory)))
   (if (and (file-exists-p templates) (boundp 'org-capture-templates))
       ;; Load work templates and report the action.
...
(defconst my/gptel-commit-system-prompt
  "You are a concise assistant that writes conventional Git commit messages. Write in imperative tone. Return only the commit message, no formatting, no comments, no explanations, and no repetition of the input. Keep the title under 72 characters. If needed, add a body after a blank line. Use ASCII only. Use double spacing after periods. Do not include code blocks. Refer to functions, commands, or package names using backticks, for example, `use-package`, `gptel`, or `magit`."
