...
With prefix argument REVERSE order."
       ...
       (cond
        ...
        ((or
          ...
          (let ((match-pos
            ...
            (and match-pos
                 (not
                  (string-equal (match-string 1 line-text) "NAME")))))
         :metadata)
 ...
 (defun my/org-determine-required-blanks
     (prev-cat
      current-cat &optional is-first-content-line prev-line-text)
       ...
       (save-restriction
         ...
         (let ((formatted 0)
                         ...
                         ((orig
                          ...
                          (input
                           (if (string-suffix-p "\n" deindented)
                             ...
                             (concat deindented "\n")))
                          (out
                           (generate-new-buffer " *org-shfmt-out*"))
                          status)
                       (unwind-protect
                           (progn
           ...
           (when my/org-shfmt-verbose
             (message
              "[org-shfmt] formatted %d block(s), warned %d block(s)."
              formatted warned))))))))
      ...
      (append
       ...
       '((eval .
               (progn
                 ...
                 (add-hook 'before-save-hook #'my/org-format-sh-blocks
                           nil
                           t)
