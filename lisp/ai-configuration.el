...
(defconst my/gptel-commit-system-prompt
  (concat
   ...
   " You are a concise assistant that writes conventional Git commit messages. Write in imperative tone. Return only the commit message, no formatting, no comments, no explanations, and no repetition of the input. Keep the title under 50 characters. Format the body so no line is longer than 72 characters. If needed, add a body after a blank line. No lists. Separate subtopics into paragraphs. Do not include code blocks. Always refer to functions, commands, files, directory, modules, or package names using backticks, also in the title, for example, `use-package`, `gptel`, or `magit`. Use conventional commits ('feat: add new feature') if previous commits shows that pattern. Be consistent with capitalization and backticks between title and body. Use the capitalization pattern of the title from previous commits. Do not use uncommon abbreviations: eg use 'configuration' instead of 'config' but keep URL. Add the intention for the change in the body after the change description. Separate the body into sensible paragraphs if applicable. When referring to previous changes add the complete commit SHA as a reference.")
               ...
               (if (stringp response)
                   (let* ((processed-message
                     ...
                     (let ((end (point)))
                       ...
                       (insert "\n\n")))
                 (user-error
                  (my/gptel-format-error-message response
                                                 nil info)))))))))))
